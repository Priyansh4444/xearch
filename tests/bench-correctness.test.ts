...
describe("bench: random-sampling correctness over the real corpus", () => {
  ...
  test.skipIf(!hasCorpus())("ingest idempotence + df conservation + L0 exactness + pagination", async () => {
  ...
  test.skipIf(!hasCorpus())("parser invariants on generated query shapes", async () => {
    const t = convexTest(schema, modules);
