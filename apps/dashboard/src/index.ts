...
// The dashboard re-lists the entire bucket per request, and /nerds auto-refreshes
// every 60s. A per-isolate snapshot cache collapses that per-viewer listing cost
// to one full scan per TTL window. Staleness is bounded by SNAPSHOT_TTL_MS and
// this is display data only — the bucket is never written here.
...
async function buildSnapshot(bucket: R2Bucket): Promise<Snapshot> {
  ...
  const value = { generatedAt: Date.now(), bucket: bucketTotals, runs };
