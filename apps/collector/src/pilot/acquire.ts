...
export const createRunEffect = Effect.fn("createRunEffect")(
  function* (
    paths: RunPaths,
    config: PilotConfig,
    runId: string,
...
export const acquireEffect = Effect.fn("acquireEffect")(
  function* (options: AcquireOptions): Effect.fn.Return<Manifest, AcquireError> {
    ...
    for (const account of checkpoint.accounts) {
      ...
      while (budgetLeft()) {
        yield* pace();
        const outcome = yield* fetchOnePageEffect(account, client, paths, checkpoint, config, now, log);
        yield* saveStateEffect(paths, checkpoint, config, now());
...
const resolveIdentityEffect = Effect.fn("resolveIdentityEffect")(
  function* (
...
const fetchOnePageEffect = Effect.fn("fetchOnePageEffect")(
  function* (
    account: AccountRecord,
    client: PilotClient,
    paths: RunPaths,
    ...
    if (userId === null) {
      return yield* Effect.die(new Error(`account ${account.requestedHandle} has no resolved user id`));
    }
    ...
    if (Exit.isFailure(pageExit)) {
      ...
      const reason: PauseReason = fxError?.kind === FxTwitterErrorKind.Decode
    ...
    for (const createdAt of authored) {
      account.oldestAuthoredCreatedAt = account.oldestAuthoredCreatedAt === null
        ? createdAt
        : Math.min(account.oldestAuthoredCreatedAt, createdAt);
      account.newestAuthoredCreatedAt = account.newestAuthoredCreatedAt === null
        ? createdAt
    ...
    if (response.page.results.length === 0) {
      ...
      if (bottom === inputCursor || account.seenCursors.includes(bottom)) {
        pause(account, PauseReason.CursorStalled, `empty page with a repeated cursor at page ${page}`, now());
