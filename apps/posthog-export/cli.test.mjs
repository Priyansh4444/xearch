...
async function fixture(t) {
  ...
  const input = path.join(dir, "export.jsonl"),
...
function mockIndexer({ offset, rejected = false, code = 0, signal = null, error, during } = {}) {
  return async (command, args, options) => {
    ...
    const rows = (await fs.readFile(path.join(dataDir, filename), "utf8"))
      .trim()
      .split("\n").length;
    const attempt = JSON.parse(
      await fs.readFile(path.join(path.dirname(checkpoint), "attempt.json"), "utf8"),
    );
    ...
    event([
      {
        ...
        reposted_by: { id: "99" },
        media: {
          photos: [{ url: "https://img.example/a" }],
          videos: [{ formats: [{ url: "https://video.example/a", container: "mp4" }] }],
          animated: [{ type: "animated_gif", url: "https://video.example/b" }],
        },
      },
    ]),
  ).records.at(-1).record;
  assert.deepEqual(
    r.media.map((x) => x.type),
    ["image", "video", "gif"],
  );
...
test("malformed versions, sizes and timestamps reject; unrelated events ignored", () => {
  ...
  assert.equal(
    normalizeEvent(event([{ ...post, created_at: "bad", created_timestamp: undefined }])).issues
      .length,
    1,
  );
});
test("stage, retry, checkpoint-gated ack, content dedup and changed metrics", async (t) => {
  ...
  await assert.rejects(
    acknowledge(f.state, batch.batch, "wrong-checkpoint", "wrong-quarantine"),
    /no longer accepts proof paths/,
  );
  await assert.rejects(
    ingest(f.state, batch.batch, { run: mockIndexer({ offset: 1 }) }),
    /complete batch/,
  );
  await assert.rejects(
    ingest(f.state, batch.batch, { run: mockIndexer({ rejected: true }) }),
    /quarantined/,
  );
  ...
  await f.write([
    event(),
    event([{ ...post, likes: 8 }], { captured_at: "2025-01-02T00:00:00Z" }),
    event([{ ...post, text: "Edited" }], { captured_at: "2025-01-03T00:00:00Z" }),
  ]);
  const next = await stage(f.input, f.state);
  assert.equal(next.records, 1);
  assert.equal(next.quarantineCount, 1);
  const output = await fs.readFile(
    path.join(next.directory, "ingress", `${next.batch}.jsonl`),
    "utf8",
  );
  ...
  await f.write(
    ["profile", "search", "followers", "following"].map((resource) =>
      event([], {
        resource,
        chunk_count: 2,
        payload: { posts: [], users: [author], profile: author },
      }),
    ),
  );
  const batch = await stage(f.input, f.state);
  assert.equal(batch.records, 1);
  assert.equal(batch.quarantineCount, 1);
});
...
test("out-of-order snapshots select newest; public URLs omit query credentials", async (t) => {
  ...
  await f.write([
    ...
    event([{ ...post, likes: 7 }], { captured_at: "2025-01-02T00:00:00Z" }),
  ]);
  const b = await stage(f.input, f.state);
  const rows = (await fs.readFile(path.join(b.directory, "ingress", `${b.batch}.jsonl`), "utf8"))
    .trim()
    .split("\n")
    .map(JSON.parse);
  assert.equal(rows.at(-1).metrics.likes, 8);
  const r = normalizeEvent(
    event([{ ...post, media: { photos: [{ url: "https://img.example/a?token=secret#secret" }] } }]),
  ).records.at(-1).record;
...
for (const mixed of [false, true]) {
  test(`unchanged newer snapshots advance cross-run watermarks (${mixed ? "mixed delta" : "zero delta"})`, async (t) => {
    ...
    const readLedger = async () =>
      JSON.parse(await fs.readFile(path.join(f.state, "ledger.json"), "utf8"));
    if (mixed) {
      // A pending delta must not advance either record's committed watermark.
      assert.equal(
        (await readLedger()).records["tweet:123"].captured,
    ...
    await f.write([
      event([{ ...post, likes: 8, author: { ...author, followers: 100 } }], {
...
test("raw export lines are bounded before parsing and oversized lines do not hide following records", async (t) => {
  ...
  const issues = (await fs.readFile(path.join(batch.directory, "quarantine.jsonl"), "utf8"))
    .trim()
    .split("\n")
    .map(JSON.parse);
  assert.deepEqual(
...
for (const failure of [{ code: 1 }, { code: null, signal: "SIGTERM" }, { error: "spawn failed" }]) {
  test(`unsuccessful process cannot acknowledge: ${JSON.stringify(failure)}`, async (t) => {
    ...
    const batch = await stage(f.input, f.state);
    await assert.rejects(
      ingest(f.state, batch.batch, { run: mockIndexer(failure) }),
      /process failed|spawn failed/,
    );
...
test("wrong quarantine path cannot be substituted for a rejected bound attempt", async (t) => {
  ...
  const clean = path.join(f.dir, "clean");
  await fs.mkdir(clean);
  await assert.rejects(
    acknowledge(f.state, batch.batch, path.join(f.dir, "checkpoint.json"), clean),
    /no longer accepts proof paths/,
  );
...
test("state stays locked while the child runs and ack validates recorded attempt hash", async (t) => {
  ...
  const latest = JSON.parse(
    await fs.readFile(path.join(batch.directory, "latest-attempt.json"), "utf8"),
  );
...
`,
    ...
    const alias = path.join(f.dir, "state-link");
    await fs.symlink(f.state, alias);
    const result = spawnSync(
      process.execPath,
      [new URL("./cli.mjs", import.meta.url).pathname, "ingest", alias, batch.batch],
      {
        cwd: f.dir,
        encoding: "utf8",
        env: { ...process.env, PATH: bin, FAKE_CARGO_MODE: mode },
        timeout: 10_000,
      },
    );
    ...
    const attempt = JSON.parse(
      await fs.readFile(
        path.join(batch.directory, "attempts", latest.attemptId, "attempt.json"),
        "utf8",
      ),
    );
...
test("empty quarantine files do not hide the clean attempt", async (t) => {
  ...
  await f.write([event()]);
  const batch = await stage(f.input, f.state);
  const result = await ingest(f.state, batch.batch, {
    run: mockIndexer({
      during: async ({ quarantine }) => {
