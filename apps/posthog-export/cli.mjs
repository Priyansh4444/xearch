...
function string(x, field) {
  requireValue(typeof x === "string" && x.trim().length > 0, `missing/invalid ${field}`);
  return x;
}
function count(x, field) {
  requireValue(Number.isSafeInteger(x) && x >= 0, `missing/invalid ${field}`);
  return x;
}
function timestamp(x) {
  const n =
    typeof x === "number" ? (x < 1e11 ? x * 1000 : x) : typeof x === "string" ? Date.parse(x) : NaN;
  requireValue(
    Number.isSafeInteger(n) && n > FLOOR && n <= Date.now() + 300_000,
    "timestamp outside (2006, now+5m]",
  );
  return n;
}
...
function author(a) {
  ...
  const out = {
    kind: "author",
    id: id(a.id),
    handle: string(a.screen_name, "screen_name").replace(/^@/, ""),
    displayName: string(a.name, "name"),
    followerCount: count(a.followers, "followers"),
    followingCount: count(a.following, "following"),
    verified,
    createdAt: timestamp(a.joined),
  };
...
function media(m) {
  ...
  return items.map(([item, fallback]) => {
    ...
    requireValue(
      ["image", "video", "gif"].includes(type) && url,
      "unsupported media type or missing URL",
    );
    return { type, url };
  });
}
function tweet(t, captured) {
  ...
  const parent =
    (!Array.isArray(t.replying_to) && t.replying_to?.status) || t.replying_to_status?.[0];
  const out = {
    kind: "tweet",
    id: id(t.id),
    text: string(t.text, "text"),
    authorId: id(t.author?.id),
    createdAt: timestamp(t.created_timestamp ?? t.created_at),
    metrics: {
      likes: count(t.likes, "likes"),
      retweets: count(t.retweets ?? t.reposts, "retweets"),
      quotes: count(t.quotes, "quotes"),
      replies: count(t.replies, "replies"),
    },
    metricsAt: captured,
    media: media(t.media),
    quotedTweetId: t.quote == null ? null : id(t.quote.id),
    retweetOfTweetId: null,
    inReplyToTweetId: parent == null ? null : id(parent),
  };
...
function fingerprint(record) {
  // metricsAt excluded: the fingerprint must not move when metrics refresh.
  const { metricsAt: _ignoredMetricsAt, ...content } = record;
  return hash(content);
}
...
export function normalizeEvent(event) {
  ...
  requireValue(
    typeof p.request_id === "string" &&
      /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(p.request_id),
    "invalid request_id",
  );
  requireValue(
    p.http_status === 200 &&
      ["tweet", "profile", "search", "followers", "following"].includes(p.resource),
    "invalid status/resource",
  );
  requireValue(
    typeof p.source === "string" && typeof p.degraded === "boolean",
    "invalid source/degraded",
  );
  requireValue(p.cache === undefined || typeof p.cache === "string", "invalid cache");
  requireValue(
    p.warnings === undefined ||
      (Array.isArray(p.warnings) && p.warnings.every((x) => typeof x === "string")),
    "invalid warnings",
  );
  requireValue(
    Number.isSafeInteger(p.chunk_count) &&
      p.chunk_count > 0 &&
      Number.isSafeInteger(p.chunk_index) &&
      p.chunk_index >= 0 &&
      p.chunk_index < p.chunk_count,
    "invalid chunk indices",
  );
  requireValue(
    typeof p.captured_at === "string" && /^\d{4}-\d{2}-\d{2}T/.test(p.captured_at),
    "invalid captured_at",
  );
  ...
  requireValue(
    Buffer.byteLength(
      JSON.stringify({
        event: event.event,
        properties: p,
        distinct_id: event.distinct_id ?? p.distinct_id,
      }),
    ) < MAX_EVENT_BYTES,
    "oversized capture event; re-export/re-chunk upstream; no truncation",
  );
  requireValue(
    object(p.payload) && Array.isArray(p.payload.posts) && Array.isArray(p.payload.users),
    "invalid payload lists",
  );
  const attempt = (raw, location, fn) => {
    try {
      records.push({ record: fn(raw), captured });
    } catch (error) {
      issues.push({ location, reason: error.message });
    }
  };
  ...
  const visit = (t, location, depth = 0) => {
    if (depth > 32) {
      issues.push({
        location,
        reason: "quote nesting exceeds 32; record retained only in original export",
      });
      return;
    }
  ...
  return {
    records,
    issues,
    chunk: { request: hash(p.request_id), index: p.chunk_index, count: p.chunk_count },
  };
}
...
async function locked(state, fn) {
  ...
  try {
    ...
    throw new Error(
      `state locked: ${lock}; if a process crashed, verify it stopped before removing lock`,
      { cause: error },
    );
  }
  try {
...
async function ledgerAt(state) {
  const ledger = await readJson(path.join(state, "ledger.json"), {
    version: 1,
    records: {},
    acked: [],
  });
  requireValue(
    ledger.version === 1 && object(ledger.records) && Array.isArray(ledger.acked),
    "unsupported/corrupt ledger",
  );
  return ledger;
}
...
export async function stage(input, state) {
  ...
  return locked(state, async () => {
    ...
    const selected = new Map(),
      issues = [],
      chunks = new Map(
        Object.entries(coverage).map(([key, group]) => [
          key,
          { count: group.count, seen: new Set(group.seen) },
        ]),
      );
    let lines = 0,
      ignored = 0,
    ...
    for await (const line of exportLines(input)) {
      ...
      if (line.oversized) {
        issues.push({
          line: lines,
          sha256: line.sha256,
          reason: "export line exceeds 1,000,000 bytes; re-export upstream; no truncation",
        });
        continue;
      }
      const raw = line.raw;
      ...
      try {
        ...
        if (group.count !== chunk.count)
          issues.push({ line: lines, reason: "inconsistent chunk_count" });
        group.seen.add(chunk.index);
        chunks.set(chunk.request, group);
        for (const entry of result.records) {
          ...
          if (
            ...
            issues.push({
              ...
              kind: r.kind,
              id: r.id,
              reason: "text changed for existing tweet ID; explicit edit policy required",
            });
            continue;
          }
          if (
          ...
          if (
            ...
            issues.push({
              ...
              kind: r.kind,
              id: r.id,
              reason: "conflicting content at same snapshot time",
            });
            continue;
          }
          selected.set(key, candidate);
        ...
        issues.push({
          line: lines,
          sha256: hash(raw),
          reason: error instanceof SyntaxError ? "invalid JSON event/properties" : error.message,
        });
      }
    }
    for (const group of chunks.values())
      if (group.seen.size !== group.count)
        issues.push({
          reason: "incomplete request chunks across observed exports",
          expected: group.count,
          received: group.seen.size,
        });
    const entries = [...selected.entries()]
      .filter(([key, x]) => ledger.records[key]?.hash !== x.hash)
      .sort(([a], [b]) => a.localeCompare(b)); // author:* precedes tweet:*
    const content = entries.map(([, x]) => JSON.stringify(x.record) + "\n").join("");
    const batch = `posthog-${Date.now()}-${randomUUID()}`;
    const temp = path.join(batches, `.${batch}.tmp`),
      target = path.join(batches, batch);
    ...
    const manifest = {
      ...
      tweets: entries.filter(([, x]) => x.record.kind === "tweet").length,
      sha256: hash(content),
      quarantineCount: issues.length,
      entries: [...selected].map(([key, x]) => ({
        key,
        hash: x.hash,
        textHash: x.textHash,
        captured: x.captured,
    ...
    await fs.writeFile(
      path.join(temp, "quarantine.jsonl"),
      issues.map((x) => JSON.stringify(x) + "\n").join(""),
      { mode: 0o600 },
    );
    await fs.rename(temp, target); // publish only a closed, complete batch
    await atomic(
      path.join(state, "chunks.json"),
      Object.fromEntries(
        [...chunks].map(([key, group]) => [key, { count: group.count, seen: [...group.seen] }]),
      ),
    );
    if (manifest.records === 0) {
      // No ingress delta, but unchanged newer observations must advance watermarks.
      for (const entry of manifest.entries)
        ledger.records[entry.key] = {
          hash: entry.hash,
...
async function batchAt(state, batch) {
  ...
  requireValue(
    manifest.batch === batch && manifest.filename === `${batch}.jsonl`,
    "manifest mismatch",
  );
...
export async function ingest(state, batch, { run = runIndexer } = {}) {
  ...
  return locked(state, async () => {
    ...
    const args = [
      ...
      "--release",
      "--manifest-path",
      path.join(repo, "indexer/Cargo.toml"),
      "--",
      "--data-dir",
      path.join(dir, "ingress"),
      "--checkpoint",
      checkpoint,
      "--quarantine",
      quarantine,
      "--lexicons",
      path.join(repo, "shared/lexicons"),
      "backfill",
    ];
    ...
    requireValue(
      succeeded,
      "indexer process failed; evidence retained; rerun ingest for a fresh attempt",
    );
...
async function acknowledgeLocked(state, batch) {
  ...
  requireValue(
    typeof latest.attemptId === "string" && /^[0-9a-f-]{36}$/.test(latest.attemptId),
    "no bound ingestion attempt; use ingest first",
  );
  ...
  requireValue(
    attempt.version === 1 &&
      ...
      attempt.batch === batch &&
      attempt.sha256 === manifest.sha256,
    "attempt does not match immutable batch",
  );
  requireValue(
    attempt.checkpoint === checkpointFile && attempt.quarantine === quarantineDir,
    "attempt paths do not match bound paths",
  );
  requireValue(
    attempt.status === "succeeded",
    "attempt did not complete successfully; rerun ingest",
  );
  const checkpoint = await readJson(checkpointFile);
  requireValue(
    typeof checkpoint.configHash === "string" || typeof checkpoint.config_hash === "string",
    "invalid indexer checkpoint",
  );
  requireValue(
    checkpoint.offsets?.[manifest.filename] === manifest.records,
    "checkpoint does not cover the complete batch; rerun ingest",
  );
  requireValue(
    (await fs.stat(quarantineDir)).isDirectory(),
    "bound quarantine path is not a directory",
  );
  for (const name of await fs.readdir(quarantineDir)) {
    ...
    requireValue(
      ...
      "indexer quarantined records or unexpected entries; resolve/retry before ack",
    );
  }
  for (const entry of manifest.entries)
    ledger.records[entry.key] = {
...
export async function acknowledge(state, batch, ...legacyPaths) {
  requireValue(
    ...
    "ack no longer accepts proof paths; use the bound ingest command",
  );
  state = await fs.realpath(state);
...
async function main() {
  ...
  if (command === "ack" && args.length > 2)
    throw new Error("ack no longer accepts proof paths; use the bound ingest command");
