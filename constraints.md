...
| `and so is` (all-stopword query) | The posting index has no entries for stopwords, so it cannot answer them | Baseline tokenizes the query with stopwords kept, retrieves them from the built-in full-text index (which indexes them) and still verifies every token. The paginated lane reads one 100-candidate window for it — a 20-candidate page is mostly rejects — and reports itself done. The posting lane shows no results and the SERP offers the literal lane. |
