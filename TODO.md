...
- [x] retire stale R2 helpers (`run-snapshot` wrote to the dead `_active/` prefix; `finish-run` / `push-status-loop` unreferenced by scripts/docs/tests) — live ops kept: `pull-run` / `push-status` / `sync-runs` / `acquire-detached`; dashboard Worker retired (apps/dashboard removed 2026-09-17, deprecated - run JSON data in R2 is untouched)
