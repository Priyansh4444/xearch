...
| K2 | **Feedback brigading** | Public demo with a vote button | Clamp ±5 per (queryKey, tweet), one vote per verified identity (client sessions are never trusted), rerank-only. Accept that determined abuse can bury one result in one query — bounded blast radius is the design, not a leftover |
